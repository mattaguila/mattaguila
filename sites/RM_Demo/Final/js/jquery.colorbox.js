	...
	function launch(target) {
		if (!closing) {
			...
			if (!open) {
				...
				$close2.html(settings.close).show();
				
				$close3.html(settings.close).show();
			}
	...
	publicMethod.init = function () {
		if (!$box) {
			...
			$content = $tag(div, "Content").append(
				...
				$close = $tag(div, "Close").addClass('showgreybox'),
				$close2 = $tag(div, "Close2"),
				$close3 = $tag(div, "Close3").addClass('addtocartbtn')
			);
