...
var lastFmRecords = (function() {
  ...
  var _defaultthumb  = './images/nocover.jpg';
  var _debug         = false;
  var _gmt_offset    = '+1';
  ...
  function _showCover(_id, _track) {
    ...
    if ('' == _track.cdcover) {
      ...
      jQuery('#lastfmcover' + _id).attr('src', _track.cdcover).parent('a').attr('href', _track.url).attr('target', '_blank').unbind('click', lastFmRecords.dontFollowLink);
    }
  };
  ...
  function _processArtistData(data) {
    ...
    jQuery.each(data, function(i, _json) {
    	...
    	jQuery('.' + _mbid).each( function() {
    	  // point src and href of parent a to the image
    	  // and make link clickable
    	  jQuery(this).attr('src', _imgurl).removeClass(_mbid).parent('a').attr('href', _json.url).attr('target', '_blank').unbind('click', lastFmRecords.dontFollowLink);
    	});
